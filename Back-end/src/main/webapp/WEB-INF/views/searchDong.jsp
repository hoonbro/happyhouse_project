...
function booking() {
	var dong = "<c:out value='${addr.dong}'/>";
	if(dong == "동" || dong == ""){
		alert("동을 먼저 검색해 주세요.");
		return;
	}
	else{
		alert("관심지역으로 등록합니다. [${addr.dong}]");
		document.getElementById("bookform").action = "${root}/member/booking";
		return;
	}
}
...
<body>
	...
	<div class="container">
		...
		<div class="col-12 text-center">
			<div>
				<div class = "col-6 text-left font-weight-bold" style="font-size : xx-large;">[${addr.dong} 아파트 거래 정보]</div>
				
				<div class = "col-12 text-right">
				<c:if test="${userinfo ne null}">
				<form id="bookform" method="post">
					<input type="hidden" name="act" id="act" value="booking">
					<input type="hidden" name="booksido" id="booksido" value="${addr.city}">
					<input type="hidden" name="bookgugun" id="bookgugun" value="${addr.gugun}">
					<input type="hidden" name="bookdong" id="bookdong" value="${addr.dong}">
					<button class = "btn btn-info" id="bookbtn" name = "bookbtn" onclick="javascript:booking();">관심지역 등록</button>
				</form>
				</c:if>
				</div>
			</div> <br>
